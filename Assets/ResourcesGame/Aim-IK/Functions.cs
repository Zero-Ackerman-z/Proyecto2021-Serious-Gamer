...
namespace AimIK.Functions
{
    ...
    public static class AimIKFunctions
    {
        ...
        public static void ScalePosition(this Transform transform, Vector3 worldPosition,float scale,float mag)
        {
            Vector3 diff = (worldPosition - transform.position);
            float dist = diff.magnitude;
            if(dist> dist*0.05f+mag)
              transform.position =Vector3.Lerp(transform.position, transform.position + diff.normalized , Time.deltaTime * scale)  ;
        }
